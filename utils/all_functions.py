...
def join_metadata (df_tuple: list, axis: int = 0, colnames: list = ['metadata_property','value'], reset_index = True)
def change_column_order (dataframe, colname: str = "type", newposition: int = 0)
...
def convert_events_xml_tag_df(filename: str, tagname='SourceTextChar', export_csv=False, csv_output='sourcetextchar.csv'):
    ...
    # convert dict to df
    event_df = pd.DataFrame.from_dict(event_data)
    # NEED to reorder and put it in the front
...
def change_column_order(dataframe, colname: str = "type", newposition: int = 0):
    dataframe.insert(0, colname, dataframe.pop(colname))
